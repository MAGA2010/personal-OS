...
## 2026-07-20｜Stage 3D-Fill Program People Coverage Expansion Wave 6

- Preflight 与范围：在 clean worktree 和包含 Wave 5 的 checkpoint `fc3ea3597f79feb18e07061efe6f1c858836f638` 上开始。Wave 6 从 immutable Candidate v2、Stage 3C program overlay 与 Waves 1–5 artifacts 自动重建 310-slot 累计状态，确认 62 identified / 248 `source_review_not_completed`；随后仅从剩余高收益 program families 中按 candidate 与 slot 的稳定 round-robin 选择 100 个 slots，覆盖 61 所学校，没有维护人工学校名单或扩大 Candidate v2。
- Reviewed intake：100 个 attempted slots 中，11 个获得 `identified_person`，89 个继续保持 `source_review_not_completed`，`no_qualifying_person_found=0`。新增 positive 覆盖 Boston College Finance、Boston University Business、Caltech Computer Science、Cornell Agricultural Economics、Princeton Economics-related Econometrics、UC Davis Computer Science、UCLA Political Science、Colorado Business、Michigan Business、Notre Dame Finance 与 UVA Computer Science。每条均由学校官方 alumni/department/profile 页面直接陈述 attendance 与 exact/related program relationship；没有从职业、公司、名气或研究方向推断专业。Harvard source 只陈述人物曾倾向 Economics，未明确证明实际修读，因此 fail closed 保留为 source gap。
- Evidence、cache 与 identity：11 条 positive records 的 attendance/program 双 anchors 共 22 个短 direct quotes，均通过 gitignored reviewed-excerpt cache、SHA-256 与 `local_cache_substring_check`；`manual_verbatim_check=0`、`cache_missing=0`。人物 ID 使用 normalized name、candidate context 与 source-backed source ID；按 `(candidate_id, canonical_person_id)` 检查累计 Waves 1–6 的 73 次正向 occurrence，得到 73 个 unique people，输入与 post-merge duplicate 均为 0。
- 累计覆盖与 gap 语义：program-person identified 从 62 增至 73 / 310，透明 `source_review_not_completed` 缺口从 248 降至 237，scoped `no_qualifying_person_found` 保持 0。attempted slots、identified people 与未完成审查分别统计；没有把未审查写成「无」。
- 工程与验证：新增独立 Wave 6 generator、fail-closed validator、generate/validate CLI、immutable input pins、reviewed source/cache/observation inputs、9 个 artifacts、report 与 8 个 TDD tests。专项测试、全量 262 项 Python tests、formal validator、fixture/schema/migration validation、artifact/report byte-identical regeneration 与 `git diff --check` 均通过；`source_policy_violations=0`、`ranking_field_contamination=0`。审查后另加 fail-closed 检查，要求 prior cumulative identities 与 62 个 prior identified slots 完全一致，并拒绝引用不存在、跨校或无缓存 source 的 scoped `no_qualifying_person_found`。
- 边界与风险：Candidate v2、ranking、Stage 3/3B/3C/3C2/3D framework、history/anecdote/attendance、Waves 1–5 与 frontend 均保持只读；未生成 final universe、formal memberships 或 frontend export。237 个 slots 仍缺 reviewed source，因此状态继续为 `source_limited / incomplete / not_final`；本 checkpoint 不声明 Gate PASS，提交后停止并等待审查，不 tag、不 push。

## 2026-07-21｜Stage 3D-Fill Program People Coverage Expansion Wave 7

- Preflight 与范围：在 clean worktree 和 Wave 6 checkpoint `ae4cc830ca687be3743a29e821a7f669b8bbf480` 上开始；Stage 3A stash 保持 untouched。Wave 7 从 immutable Candidate v2、Stage 3C program overlay 与 Waves 1–6 artifacts 自动重建 310-slot 累计状态，确认 73 identified / 237 `source_review_not_completed`；随后用 Tier priority 与 school-spread round robin 自动选取 100 个剩余高价值 slots，覆盖 56 所学校，没有维护人工学校名单或扩大 Candidate v2。
- Reviewed intake：100 个 attempted slots 中，27 个获得 `identified_person`，73 个继续保持 `source_review_not_completed`，`no_qualifying_person_found=0`。正向记录覆盖 CS、Mechanical/Aerospace Engineering、Business、Economics/Finance、Biology、Data Science、Government 等已有 demo program；每条均由学校官方 alumni、department、archive、award 或 profile 页面直接陈述 attendance 与 exact/related program relationship，没有用职业、公司、职位、研究方向或名气推断专业。
- Evidence、cache 与 identity：27 条 positive records 的 attendance/program 双 anchors 共 54 个短 direct quotes，全部通过 gitignored reviewed-excerpt cache、SHA-256 与 `local_cache_substring_check`；`manual_verbatim_check=0`、`cache_missing=0`。人物 ID 使用 normalized name、candidate context 与 source-backed disambiguator；按 `(candidate_id, canonical_person_id)` 检查累计 Waves 1–7 的 100 次正向 occurrence，得到 100 个 unique people，输入与 post-merge duplicate 均为 0。
- 累计覆盖与 gap 语义：program-person identified 从 73 增至 100 / 310，透明 `source_review_not_completed` 缺口从 237 降至 210，scoped `no_qualifying_person_found` 保持 0。attempted slots、identified people 与未完成审查分别统计；gap 不携带人物且不渲染成「无」。
- 工程与验证：新增独立 Wave 7 generator、fail-closed validator、additive generate/validate CLI、immutable input pins、reviewed source/cache/observation inputs、独立 artifacts/report 与 16 个 TDD tests。红灯由 Wave 7 module、inputs 与 CLI 缺失产生；实现及审查加固后专项 16/16、全量 Python 278/278、formal Wave 7 validator、fixture/schema/migration、artifact/report byte-identical regeneration 与 `git diff --check` 均通过。validator 覆盖 remaining-slot derivation、dual evidence、source-stated match、candidate-bound HTTPS official host、非空 identity、cache SHA/substring 与 ranking-field scan、gap/exclusion provenance、cumulative dedup/dashboard、固定 Wave 6 baseline 的记录式 clean preflight、validation-result drift、精确 artifact 目录与 CLI output collision、Waves 1–6 immutability和 deterministic regeneration。
- 边界与风险：Candidate v2、ranking、Stage 3/3B/3C/3C2/3D framework、history/anecdote/attendance、Waves 1–6 与 frontend 均保持只读；未生成 final universe、formal memberships、frontend/preview export 或 Stage 4A overlay。210 个 slots 仍缺 reviewed source，因此状态继续为 `source_limited / incomplete / not_final`；本 checkpoint 不声明 Gate PASS，提交后停止并等待审查，不 tag、不 push。

## 2026-07-21｜Stage 3D-Fill Program People Coverage Expansion Wave 8

- Preflight 与范围：在 clean worktree、Wave 7 Gate checkpoint `f0530398a02b3bbecfd5347487c9b6eb3dc0c0af` 及其 100-coverage tag 上开始；Stage 3A stash 保持 untouched。Wave 8 从 immutable Candidate v2、Stage 3C program overlay 与 Waves 1–7 artifacts 自动重建 310-slot 累计状态，确认 100 identified / 210 `source_review_not_completed`；再按 Tier priority 与 school-spread round robin 自动选取 100 个 remaining slots，没有手写学校清单、扩大 Candidate v2 或修改旧 wave。
- Reviewed intake：100 个 attempted slots 中，25 个获得 `identified_person`，75 个继续保持 `source_review_not_completed`，`no_qualifying_person_found=0`。新增 positive 分布于 24 所学校，覆盖 Computer Science、Civil/Environmental/Industrial/Mechanical/Biomedical Engineering、Economics/Finance、MIS、Government 与 Biology；仅接受官方 alumni、department、college、archive 或 award pages 对 attendance 和 exact/related program 的明确陈述，未从职业、公司、职位、研究方向或名气推断专业。
- Evidence、cache 与 identity：25 条 positive records 的 attendance/program 双 anchors 共 50 个短 direct quotes，均进入 gitignored reviewed-excerpt cache并通过 SHA-256 与 `local_cache_substring_check`；`manual_verbatim_check=0`、`cache_missing=0`，cache 正文未提交。人物 ID 由 normalized name、candidate context 与 source-backed disambiguator 构成；按 `(candidate_id, canonical_person_id)` 检查累计 125 次正向 occurrence，得到 125 个 unique people，输入与 post-merge duplicate 均为 0。
- 累计覆盖与 gap 语义：program-person identified 从 100 增至 125 / 310，透明 `source_review_not_completed` 缺口从 210 降至 185，scoped `no_qualifying_person_found` 仍为 0。attempted slots、identified people 和未完成审查分别统计；gap 不携带人物且不渲染成「无」。
- 工程与 validator：新增独立 Wave 8 generator、fail-closed validator、additive generate/validate CLI、immutable input pins、reviewed source/cache/observation inputs、独立 artifacts/report 与 16 个 TDD tests。validator 覆盖 remaining-slot derivation、school spread、dual evidence、source-stated match、candidate-bound HTTPS official host、identity、cache SHA/substring、gap semantics、cumulative dedup/dashboard、validation-result drift、旧 waves immutability、ranking isolation 与 deterministic regeneration。
- 边界与风险：Candidate v2、ranking、Stage 3/3B/3C/3C2/3D framework、history/anecdote/attendance、Waves 1–7 和 frontend 均保持只读；未生成 final universe、formal memberships、frontend/preview export 或 Stage 4A overlay。185 个 slots 仍缺 reviewed source，因此状态保持 `source_limited / incomplete / not_final`；本 checkpoint 不声明 Gate PASS，不 tag、不 push，提交后等待 combined Gate review。

## 2026-07-21｜Stage 3D-Fill Program People Accelerated Coverage Expansion Wave 9

- Preflight 与范围：在 clean worktree、Wave 8 Gate checkpoint `b1aa120462a096dc5ff7b1dde32bcffbf9ecd99a` 及 `pathos-stage3d-program-people-125-coverage-pass` tag 上开始；Stage 3A stash 保持 untouched。Wave 9 从 immutable Candidate v2、Stage 3C program overlay 与 Waves 1–8 artifacts 自动重建 310-slot 累计状态，确认 125 identified / 185 `source_review_not_completed`；加速模式仅将 attempt cap 提高到 150，仍按 Tier priority 与 school-spread round robin 选择 remaining slots，覆盖 59 所学校。
- Reviewed intake：150 个 attempted slots 中，25 个获得 `identified_person`，125 个继续保持 `source_review_not_completed`，`no_qualifying_person_found=0`。正向记录使用学校官方 alumni、department、college、news、archive 或 commencement pages；同一 reviewed source 明确支持 attendance 与 exact/related program，没有使用职业、公司、职位、名气或研究方向反推专业。
- Evidence、cache 与 identity：25 条 positive records 的 attendance/program anchors 共 50 次验证，全部通过 gitignored reviewed-excerpt cache、SHA-256 与 `local_cache_substring_check`；`manual_verbatim_check=0`、`cache_missing=0`，cache 正文未进入 commit。canonical person ID 由 normalized name、candidate context 与 source-backed disambiguator 组成，禁止 name-only ID 与 fuzzy merge。
- 累计覆盖与去重：program-person identified 从 125 增至 150 / 310，透明 `source_review_not_completed` 缺口从 185 降至 160，scoped `no_qualifying_person_found` 仍为 0。按 `(candidate_id, canonical_person_id)` 检查累计 150 次正向 occurrence，得到 150 个 unique people，输入 duplicate 和 post-merge duplicate 均为 0。
- 工程与 validator：新增独立 Wave 9 generator、fail-closed validator、additive generate/validate CLI、immutable input pins、reviewed source/cache/observation inputs、10 个 artifacts、report 与 11 个 TDD tests。validator 保持 Wave 8 的 dual-evidence、source-stated match、candidate-bound official HTTPS host、identity、cache SHA/substring、gap semantics、cross-wave dedup、ranking isolation、old-wave immutability 与 deterministic regeneration 标准；accelerated mode 未降低 evidence policy。专项 tests 11/11、全量 Python tests 305/305、formal Wave 9 validator 与 fixture/schema/migration validation 均通过。
- 边界与风险：Candidate v2、ranking、Stage 3/3B/3C/3C2/3D framework、history/anecdote/attendance、Waves 1–8 和 frontend 均保持只读；未生成 final universe、formal memberships、frontend/preview export 或 Stage 4A overlay。160 个 slots 仍缺 reviewed source，状态继续为 `source_limited / incomplete / not_final`；本 checkpoint 不 tag、不 push，完成后停止并等待 Wave 10 或 combined Gate decision。

## 2026-07-21｜Stage 3D-Fill Program People Accelerated Coverage Expansion Wave 10

- Preflight 与范围：在 clean worktree 和 Wave 9 checkpoint `1f71d03c248fa3b3aec26ecb920575a61adee953` 上开始，确认 Wave 8 的 `pathos-stage3d-program-people-125-coverage-pass` tag 保持不变，Stage 3A stash untouched，且没有 audit/helper 残留。Wave 10 从 immutable Candidate v2、Stage 3C program overlay 与 Waves 1–9 artifacts 自动重建 310-slot 累计状态，确认 150 identified / 160 `source_review_not_completed`；accelerated attempt cap 为 160，仅处理这 160 个 remaining slots。
- Reviewed intake：160 个 attempted slots 中，30 个获得 `identified_person`，130 个继续保持 `source_review_not_completed`，`no_qualifying_person_found=0`。正向记录分布于 30 所学校，来自学校官方 alumni、department、college、news、directory 或 profile pages；每条来源直接支持 attendance 与 exact/related program relationship，没有使用职业、公司、职位、名气、研究方向或普通 alumni 身份反推专业。
- Evidence、cache 与 identity：30 条 positive records 的 attendance/program 双 anchors 共 60 个短 direct quotes，全部通过 gitignored reviewed-excerpt cache、SHA-256 与 `local_cache_substring_check`；`manual_verbatim_check=0`、`cache_missing=0`，cache 正文未进入 commit。canonical person ID 由 normalized name、candidate context 与 source-backed disambiguator 构成，禁止 name-only ID、fuzzy merge 与同名跨 context 自动合并。
- 累计覆盖与去重：program-person identified 从 150 增至 180 / 310，透明 `source_review_not_completed` 缺口从 160 降至 130，scoped `no_qualifying_person_found` 仍为 0。按 `(candidate_id, canonical_person_id)` 检查累计 180 次正向 occurrence，得到 180 个 unique people，输入 duplicate 与 post-merge duplicate 均为 0；达到 180 milestone 不改变非最终语义。
- 工程与 validator：新增独立 Wave 10 generator、fail-closed validator、additive generate/validate CLI、immutable input pins、reviewed source/cache/observation inputs、独立 artifacts/report 与 12 个 TDD tests。validator 覆盖 remaining-slot derivation、160-slot cap、school spread、dual evidence、source-stated match、official HTTPS source policy、identity、cache SHA/substring、gap semantics、cross-wave dedup、cumulative dashboard、validation-result drift、旧 Waves 1–9 immutability、ranking isolation、accelerated evidence parity 与 deterministic regeneration。专项 tests 12/12、全量 Python tests 317/317 通过；formal validator、fixture/schema/migration、byte-identical regeneration 与 `git diff --check` 在提交前继续执行并记录。
- 边界与风险：Candidate v2、ranking、Stage 3/3B/3C/3C2/3D framework、history/anecdote/attendance、Waves 1–9 与 frontend 均保持只读；未生成 final universe、formal memberships、frontend/preview export 或 Stage 4A overlay。130 个 slots 仍缺 reviewed source，状态保持 `source_limited / incomplete / not_final`；本 checkpoint 不声明 combined Gate PASS，不 tag、不 push，提交后停止并等待 Wave 9 + Wave 10 combined Gate review。

## 2026-07-22｜Stage 3D Closing Hardening：Program People Provenance Closure

- Preflight 与冻结范围：在 clean worktree、Wave 10 checkpoint `e4b3dcf5621dcaa498c4e3e7cd714004fedcbd9f` 和 `pathos-stage3d-program-people-180-coverage-pass` tag 上开始；Stage 3A stash 保持 untouched。新增 54 项 immutable input pins，固定 Candidate v2、Stage 3C top-5 programs、People/Narrative coverage summaries，以及 Waves 1–10 program-person、source/cache manifest、gap 和 summary inputs。累计状态经重建确认仍为 62 校、310 slots、180 identified、130 `source_review_not_completed`、0 `no_qualifying_person_found`、180 raw/unique people、0 duplicate。
- Live intake：对 180 个唯一原始官方 source URLs 完成有界 re-fetch，写入独立 gitignored closing cache；live intake 与离线 artifact generation 明确分层。结果为 107 `live_verified_exact`、16 `live_verified_normalized`、14 `live_page_changed_review_required`、41 `live_unavailable`、2 `live_not_found`、0 `live_source_mismatch`。changed/unavailable/not-found 均只形成 findings，不自动改写或判 invalid；2 个 404 和 14 个 changed 项等待 Closing Gate 人工判断。
- Anchor hardening：扫描 360 个 attendance/program anchors，保守规则标记 136 个 thin anchors、涉及 77 条 records。100 个 anchors 可从同一原 cache 的逐字 span 安全扩宽并保留原 quote；36 个无法安全扩宽，继续保留原 quote 并标记 `anchor_hardening_unresolved`。overlay 不改变 person、university、slot、relationship、match type 或 source-stated basis，也不跨 source 拼接。
- Cache inventory：交叉检查 Waves 1–10 source/cache manifests、positive records 与实际 wave cache directories，确认 missing referenced cache 为 0；识别 11 个 orphan files、1 组 identical-content files、1 个被 4 条 records 共享的 cache。cleanup plan 对所有 orphan 保持 `safe_to_delete=false` 与 `manual_review_required=true`；未执行删除、未修改旧 manifest 或旧 cache。
- Cumulative closing summary：History、Anecdotes、Notable Attendance 均为 62/62；Program People 为 180/310（58.06%），剩余 130 gaps 不携带人物且 `display_as_none=false`。`source_policy_violations=0`、`ranking_field_contamination=0`、post-merge duplicate=0；状态继续为 `source_limited / incomplete / not_final`，不代表 final universe、official memberships 或 frontend/export readiness。
- 工程与边界：新增独立 closing data/artifact namespace、deterministic generator、fail-closed validator、单一 additive CLI（freeze/intake/generate/validate modes）、报告与 16 个 TDD tests。网络只在 intake mode 使用；generate/validate 在 network-disabled 环境中读取冻结 metadata/cache。Waves 1–10、Candidate v2、ranking、上游 Stage artifacts、frontend 与旧 cache 均保持只读；未 tag、未 push、未生成 final/preview/production/Stage 4A export。完成全部本地验证后提交，并等待独立 Stage 3D Closing Gate Review。

## 2026-07-24｜Stage 4A：Frontend Handoff Reconciliation 与产品数据缺口审计

- 统计口径澄清：handoff extractor 的 35 条 mock/demo 是 extraction-level observations，Stage 4A 的 24 条 `mock_demo_placeholder` 是 normalized staging records，两者不可直接比较；`derived_display_value` 与 `frontend_ui_only` 继续独立分类。冲突统计同样区分 4 个 authoritative backend/frontend disagreement groups 与 12 个包含 conflict 或 missing-backend state 的 field groups。本次仅补充文档，不改变 7,132 条输入、6,084 条 quarantine、1,053 条 conflicts、0 条 verified contribution 或 Gate 结论。

- 范围与输入：在 `feature/stage4a-frontend-handoff-reconciliation` 独立分支上，将外部只读输入 `handoff/frontend-data-extraction` 的 26 个文件（15,968,327 bytes）逐文件计算 SHA-256。handoff manifest、7,132 条 inventory、classification/category counts 与 7,132 条 source path / symbol / line trace 均一致；未发现常见 secret、`.env`、构建目录或二进制内容。原 `handoff/` 未提交、未修改，仅在本地 `.git/info/exclude` 加入 `/handoff/`。
- Raw → staging：新增 committed raw snapshot，保存提取 manifest、完整 record inventory、normalized previews、原始路径、逐文件 SHA，以及 `not_authoritative=true` / `requires_backend_reconciliation=true`。staging 将 7,132 条记录标准化为 field、scope、Candidate v2 identity、source/conflict 状态；62 个 frontend university names 均通过 exact canonical name 高置信匹配 Candidate v2，未使用 fuzzy matching。
- Reconciliation：后端 verified 数据保持权威。前端硬编码中的 mock/fallback、UI config、derived display policy、demonstration regional estimates、缺少 scope/year/source 的 cost/quality/nearby 数据和 ranking conflicts 均未进入 verified overlay。由于没有 frontend-only MVP 候选同时提供 primary source、明确 scope 和 reference year，本轮 verified contribution 为 0；6,084 条需隔离记录按类别进入 quarantine，1,053 条 conflict records 保持未解决。没有用前端人物填补 130 个 program-person gaps。
- 产品覆盖：学校 identity、aliases、city/state/region、coordinates、top-five programs、tuition、student-faculty ratio、nearest towns、History、Anecdotes、Notable Attendance 与 source/data-quality status 可由现有 backend layer 提供。Program People 保持 180/310 identified、130 `source_review_not_completed`、0 `no_qualifying_person_found`、180 raw/unique、0 duplicate。Chinese name、school type、enrollment、acceptance rate、SAT、TOEFL 与区域 income/crime/safety/Asian/Chinese population/cost/rent/density/transport 仍需独立来源；区域事实未混入 university entity，Chinese ratio 未以 Asian ratio 代替。
- 工程与边界：新增 deterministic intake/generator/validator CLI、12 个 Stage 4A artifacts、两份报告、数据覆盖矩阵、missing-data report、P0/P1/P2 backlog 与 15 个 TDD tests。artifact generation 完全离线，verified enrichment 是独立空 overlay，不回写 Candidate v2、ranking、Stage 3/3B/3C/3C2/3D、Waves 1–10 或 Closing artifacts；frontend 保持只读，未生成 final universe、memberships 或 frontend/preview/production export。
- 风险与下一步：核心 POI、基础详情和来源面板具备数据基础，但 choropleth、完整筛选/比较和 parent/AI 场景被区域指标及若干学校字段阻塞。Stage 4A 仍为 `source_limited / incomplete / not_final`；下一步应进行独立 Gate Review，再按 backlog 优先补 P0 acceptance rate / marker adapter 与 P1 IPEDS/College Scorecard、ACS 和安全模型数据。

## 2026-07-24｜Stage 4B：Unified Official Product Data Completion

- 范围与冻结输入：在 Stage 4A checkpoint `24ae71cc84c5e11aa6b0cc76392f0408d995a7be` 上创建独立 Stage 4B 分支。immutable pins 固定 Candidate v2 的 62 所学校、ranking memberships、Stage 3/3B/3C/3C2、Stage 3D Closing、Stage 4A、schema/migrations 与 People 180 identified / 130 transparent gaps；外部 `handoff/` 保持未追踪和只读。
- 官方学校与招生数据：复用冻结的官方 College Scorecard 2025 release 和 IPEDS HD2024 cache。62/62 获得 school control/type、undergraduate degree-seeking enrollment、acceptance rate、150% graduation rate与 first-year retention；SAT/ACT middle-50 证据各覆盖 53/62。graduate/total enrollment、test-optional 与 TOEFL policy 因冻结输入不支持而保持 pending，未从分数或前端硬编码推断。
- 地理与区域语义：62/62 获得官方 county GEOID 与 CBSA，46/62 获得经 Stage 3C2 reviewed campus-place join；16 个 place join 保持 pending。Census geocoder 请求被执行网关拒绝，ACS API 要求当前不可用的 key，因此 income、rent、density、Asian/Chinese ratio 仍 deferred。Chinese ratio 未用 Asian ratio 替代，州级数据未冒充城市数据，nearest town 未作为 campus geography。
- 生活与产品 contract：crime 与 safety index 分离，缺失 crime 不视为安全；cost-of-living 不造 aggregate score。transport 仅以 verified nearest-three-town distance 形成 62/62 partial evidence，机场、铁路与公共交通保持 deferred。新增 marker、search、filter、comparison、parent/student mode 与 AI context contracts；null 不转为 0，quarantine 不进入 search/AI，mode relevance 不伪装为客观权重。
- Overlay 与覆盖：独立 verified enrichment overlay 产生 710 条 sourced observations，source-policy violations、ranking contamination、duplicate overlay 与 missing referenced cache 均为 0。History/Anecdotes/Notable Attendance 保持 62/62，Program People 保持 180/310 identified、130 `source_review_not_completed`、0 duplicate。没有从 frontend hardcoded values 生成 verified facts。
- 工程与验证：新增 8 个 Stage 4B config/policy files、独立 raw intake metadata、26 个 deterministic artifacts、3 份报告、分层 generator/validator/additive CLI 与 Stage 4B TDD suite。fail-closed validator 执行 60 项检查，覆盖 input pins、source/cache provenance、字段 scope/year/unit、school/region isolation、admissions semantics、product contracts、gap policy、upstream/frontend isolation和无 export 边界；generate/validate 均可在网络禁用环境重复执行。
- 风险与下一步：core map POI 与 school-detail preview contract 已具备，但 choropleth、完整 regional comparison、parent regional claims 与部分 filters 仍被 ACS、crime/safety、COL、TOEFL/test-policy 和 enrollment scope 缺口阻塞。本阶段未生成 final universe、memberships、frontend/preview/production export，不修改 frontend 或旧 artifacts；状态继续为 `source_limited / incomplete / not_final`，下一步是独立 Unified Gate Review。

## 2026-07-24｜Stage 4C：MVP-Critical Data Completion

- 范围与冻结输入：在 Stage 4B checkpoint `c70b1e721ba3dcfaf79dd4327bdee955e909d3e0` 上建立独立 Stage 4C overlay。immutable pins 固定 Candidate v2、ranking memberships、Stage 3D Closing、Stage 4B data/artifacts/reports、官方 Scorecard/IPEDS/Census 缓存及 schema/migrations；原始 handoff 保持只读、未追踪且逐文件 SHA-256 不变。
- Enrollment 与招生语义：从冻结的 College Scorecard release 获取 62/62 undergraduate enrollment；60 所学校具有 graduate enrollment，并仅在 undergraduate/graduate source、year、institution scope 一致时生成 total enrollment。Harvey Mudd College 与 Olin College 的 graduate/total 保持明确 `not_reported`。SAT/ACT middle-50 维持 53/62 verified，其余 9 校使用 null + `not_reported`，未以 0 或平均数替代。
- Policy 与 localization：为 62 校生成 current undergraduate test-policy 和 international undergraduate English-policy observations；由于本次没有冻结到可复现的学校官方 policy pages，全部保持 `pending_external_access`，没有从 SAT presence、记忆或前端硬编码推断政策。新增 62/62 reviewed established Chinese display names，仅作为 localization alias，不参与 Candidate identity 或学校匹配。
- Geography、ranking 与区域数据：复用 46 个 verified Census place joins；其余 16 校明确采用 `county_only_valid` fallback，禁止 nearest-town substitution。National ranking 保持 50 条既有 rank，12 校以 `not_in_current_national_scope` 表达 null，不使用 rank 0。ACS API 返回 Missing Key、官方 bulk table 经网关返回 HTTP 403，IPEDS ADM 下载超时，因此 income、rent、density、Asian/Chinese ratio 均保持 `pending_external_access`；Chinese ratio 未由 Asian ratio替代，crime/safety/COL 也未造值。
- Overlay 与产品 readiness：新增 194 条 verified Stage 4C enrichment records，均有 scope、reference year、unit 与 source provenance；与 Stage 4B 的 710 条合并后 cumulative verified records 为 904，重复为 0。Core map、search、source panel ready；school detail、admissions、filters、comparison ready with warnings；choropleth 仍 blocked。Preview readiness contract 已生成，但未生成 preview、production 或 frontend export。
- Validator 与测试：新增分层 generator、86 项 fail-closed validator、additive CLI、28 个 deterministic artifacts、3 份报告和 26 个 TDD tests。覆盖 enrollment scope、policy enum、English undergraduate boundary、localization identity isolation、SAT/ACT null semantics、county fallback、ranking null semantics、regional source failure、cache/input pins、People 180/130 preservation、upstream/frontend isolation和 network-disabled regeneration。
- 边界与下一步：Candidate v2、ranking memberships、Stage 4B、Stage 3D People、frontend 与 handoff 均未修改；Program People 保持 180/310 identified 与 130 transparent gaps。未生成 final universe、official memberships 或任何 export。本阶段继续为 `source_limited / incomplete / not_final`；完成全量验证并提交后，应进入 Stage 4B + 4C Combined Gate，再决定是否启动 warning-aware Preview Adapter。
