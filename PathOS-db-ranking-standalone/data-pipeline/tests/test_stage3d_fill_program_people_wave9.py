"""TDD contracts for accelerated Stage 3D-Fill Program People Wave 9."""
...
try:
    from pathos_data.stage3d_fill_program_people_wave9 import (
        Stage3DFillProgramPeopleWave9ValidationError,
        build_stage3d_fill_program_people_wave9,
        validate_preflight_state,
        validate_stage3d_fill_program_people_wave9,
    )
except ImportError:
    Stage3DFillProgramPeopleWave9ValidationError = ValueError
    build_stage3d_fill_program_people_wave9 = None
    validate_preflight_state = None
    validate_stage3d_fill_program_people_wave9 = None


ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data/stage3d-fill-program-people-wave9"
...
EXPECTED_HEAD = "b1aa120462a096dc5ff7b1dde32bcffbf9ecd99a"
...
class Stage3DFillProgramPeopleWave9Tests(unittest.TestCase):
    def setUp(self):
    ...
    def _pins_for(self, **paths):
        ...
        pin_ids = {
            "source_manifest_path": "wave9_source_manifest_input",
            "cache_manifest_path": "wave9_cache_manifest_input",
            "observations_path": "wave9_observations_input",
            "exclusions_path": "wave9_exclusions_input",
        }
    ...
    def test_preflight_rejects_dirty_or_stale_wave8_baselines(self):
        self.assertIsNotNone(validate_preflight_state)
        validate_preflight_state(EXPECTED_HEAD, "", EXPECTED_HEAD)
        with self.assertRaises(Stage3DFillProgramPeopleWave9ValidationError):
    ...
    def test_remaining_slots_are_derived_and_accelerated_cap_is_enforced(self):
        self.assertIsNotNone(build_stage3d_fill_program_people_wave9)
        artifacts = build_stage3d_fill_program_people_wave9(**self._inputs())
        plan = artifacts["stage3d-fill-program-people-wave9-plan.json"]
        rows = artifacts["stage3d-fill-program-people-wave9-program-people.json"]["slots"]
        self.assertEqual(plan["remaining_slots_before_wave9"], 185)
        self.assertGreater(plan["attempted_remaining_slots"], 100)
        self.assertLessEqual(plan["attempted_remaining_slots"], 150)
        self.assertTrue(plan["accelerated_mode"])
        self.assertEqual(plan["accelerated_slot_cap"], 150)
        self.assertEqual(
    ...
    def test_inference_bases_remain_rejected_in_accelerated_mode(self):
        ...
        for basis in ("career_inferred", "company_inferred", "fame_inferred", "research_direction_inferred"):
            ...
            with self.subTest(basis=basis), self.assertRaises(Stage3DFillProgramPeopleWave9ValidationError):
                build_stage3d_fill_program_people_wave9(**self._inputs(
    ...
    def test_manual_missing_cache_and_sha_mismatch_are_rejected(self):
        ...
        missing["entries"][0]["cache_path"] = "cache/stage3d-fill-program-people-wave9/missing.txt"
        ...
        for name, document in mutations:
            ...
            with self.subTest(name=name), self.assertRaises(Stage3DFillProgramPeopleWave9ValidationError):
                build_stage3d_fill_program_people_wave9(**self._inputs(
    ...
    def test_gap_semantics_never_render_unreviewed_as_none(self):
        ...
        artifacts = build_stage3d_fill_program_people_wave9(**self._inputs())
        gaps = artifacts["stage3d-fill-program-people-wave9-gap-disclosure.json"]
    ...
    def test_cross_wave_dedup_and_dashboard_are_consistent(self):
        ...
        self.assertEqual(summary["prior_cumulative_identified_person_count"], 125)
        ...
        self.assertEqual(summary["coverage_delta_from_wave8"], summary["newly_identified_person_count"])
    ...
    def test_deterministic_regeneration_and_validator(self):
        first = build_stage3d_fill_program_people_wave9(**self._inputs())
        second = build_stage3d_fill_program_people_wave9(**self._inputs())
        self.assertEqual(first, second)
        result = validate_stage3d_fill_program_people_wave9(first, **self._inputs())
        self.assertEqual(result["status"], "passed")
        invalid = deepcopy(first)
        invalid["stage3d-fill-program-people-wave9-summary.json"]["ranking_field_contamination"] = 1
        with self.assertRaises(Stage3DFillProgramPeopleWave9ValidationError):
            validate_stage3d_fill_program_people_wave9(invalid, **self._inputs())
    ...
    def test_cli_generates_and_validates_independent_overlay(self):
        ...
        generated = subprocess.run([
            sys.executable, "-m", "pathos_data", "generate-stage3d-fill-program-people-wave9",
            *shared, "--output", str(output), "--report-output", str(report),
        ], cwd=ROOT, env=env, capture_output=True, text=True)
        self.assertEqual(generated.returncode, 0, generated.stderr)
        result = self.temp / "validation.json"
        validated = subprocess.run([
            sys.executable, "-m", "pathos_data", "validate-stage3d-fill-program-people-wave9",
