"""TDD contracts for Stage 3D-Fill Program People Coverage Expansion Wave 8."""
...
try:
    from pathos_data.stage3d_fill_program_people_wave8 import (
        Stage3DFillProgramPeopleWave8ValidationError,
        build_stage3d_fill_program_people_wave8,
        validate_preflight_state,
        validate_stage3d_fill_program_people_wave8,
    )
except ImportError:
    Stage3DFillProgramPeopleWave8ValidationError = ValueError
    build_stage3d_fill_program_people_wave8 = None
    validate_preflight_state = None
    validate_stage3d_fill_program_people_wave8 = None


ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data/stage3d-fill-program-people-wave8"
...
EXPECTED_HEAD = "f0530398a02b3bbecfd5347487c9b6eb3dc0c0af"
...
class Stage3DFillProgramPeopleWave8Tests(unittest.TestCase):
    def setUp(self):
    ...
    def _pins_for(self, **paths):
        ...
        pin_ids = {
            "source_manifest_path": "wave8_source_manifest_input",
            "cache_manifest_path": "wave8_cache_manifest_input",
            "observations_path": "wave8_observations_input",
            "exclusions_path": "wave8_exclusions_input",
        }
    ...
    def test_preflight_rejects_dirty_or_stale_wave7_baselines(self):
        self.assertIsNotNone(validate_preflight_state)
        validate_preflight_state(EXPECTED_HEAD, "", EXPECTED_HEAD)
        with self.assertRaises(Stage3DFillProgramPeopleWave8ValidationError):
    ...
    def test_remaining_slots_are_derived_with_school_spread_and_priority(self):
        self.assertIsNotNone(build_stage3d_fill_program_people_wave8)
        artifacts = build_stage3d_fill_program_people_wave8(**self._inputs())
        plan = artifacts["stage3d-fill-program-people-wave8-plan.json"]
        rows = artifacts["stage3d-fill-program-people-wave8-program-people.json"]["slots"]
        self.assertEqual(plan["remaining_slots_before_wave8"], 210)
    ...
    def test_identified_records_require_dual_source_stated_cache_evidence(self):
        ...
        identified = [
            row
            for row in artifacts["stage3d-fill-program-people-wave8-program-people.json"]["slots"]
            if row["slot_status"] == "identified_person"
        ]
        self.assertGreater(len(identified), 0)
    ...
    def test_missing_cache_and_sha_mismatch_are_rejected(self):
        ...
        missing["entries"][0]["cache_path"] = (
            "cache/stage3d-fill-program-people-wave8/missing.txt"
        )
    ...
    def test_empty_person_identity_and_incomplete_exclusion_are_rejected(self):
        ...
        exclusions = {
            "record_type": "stage3d_fill_program_people_wave8_exclusions_input",
    ...
    def test_gap_semantics_fail_closed_and_never_render_unreviewed_as_none(self):
        ...
        artifacts = build_stage3d_fill_program_people_wave8(**self._inputs())
        gaps = artifacts["stage3d-fill-program-people-wave8-gap-disclosure.json"]
    ...
    def test_cross_wave_dedup_is_unique_and_validator_fails_closed(self):
        ...
        invalid_dedup = invalid["stage3d-fill-program-people-wave8-dedup-report.json"]
        invalid_dedup["records"].append(deepcopy(invalid_dedup["records"][0]))
        with self.assertRaises(Stage3DFillProgramPeopleWave8ValidationError):
            validate_stage3d_fill_program_people_wave8(invalid, **self._inputs())
    ...
    def test_all_prior_wave_artifacts_are_immutable(self):
        ...
        build_stage3d_fill_program_people_wave8(**self._inputs())
        after = {
            str(path): hashlib.sha256(path.read_bytes()).hexdigest()
            for directory in PRIOR_WAVE_DIRS
            for path in directory.glob("*.json")
    ...
    def test_cumulative_summary_accounts_for_all_310_slots(self):
        ...
        self.assertEqual(summary["prior_cumulative_identified_person_count"], 100)
        ...
        self.assertEqual(
            summary["coverage_delta_from_wave7"],
    ...
    def test_deterministic_regeneration_and_validator(self):
        first = build_stage3d_fill_program_people_wave8(**self._inputs())
        second = build_stage3d_fill_program_people_wave8(**self._inputs())
        self.assertEqual(first, second)
        result = validate_stage3d_fill_program_people_wave8(first, **self._inputs())
        self.assertEqual(result["status"], "passed")
        invalid = deepcopy(first)
        invalid["stage3d-fill-program-people-wave8-summary.json"][
    ...
    def test_cli_generates_and_validates_independent_overlay(self):
        ...
        committed_validation = output / "stage3d-fill-program-people-wave8-validation-result.json"
