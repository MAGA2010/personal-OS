"""High-yield reviewed program-person expansion for Stage 3D-Fill Wave 8."""
...
PIN_RECORD_TYPE = "stage3d_fill_program_people_wave8_immutable_input_pin_manifest"
TOTAL_CANDIDATE_SCHOOLS = 62
TOTAL_PROGRAM_SLOTS = 310
ATTEMPTED_SLOTS = 100
PRIOR_IDENTIFIED_SLOTS = 100
PRIOR_PENDING_SLOTS = 210
EXPECTED_WAVE7_HEAD = "f0530398a02b3bbecfd5347487c9b6eb3dc0c0af"
OUTPUT_FILES = (
    "stage3d-fill-program-people-wave8-plan.json",
    "stage3d-fill-program-people-wave8-program-people.json",
    "stage3d-fill-program-people-wave8-exclusions.json",
    "stage3d-fill-program-people-wave8-source-manifest.json",
    "stage3d-fill-program-people-wave8-cache-manifest.json",
    "stage3d-fill-program-people-wave8-gap-disclosure.json",
    "stage3d-fill-program-people-wave8-cumulative-dedup.json",
    "stage3d-fill-program-people-wave8-dedup-report.json",
    "stage3d-fill-program-people-wave8-summary.json",
)
VALIDATION_FILE = "stage3d-fill-program-people-wave8-validation-result.json"
...
class Stage3DFillProgramPeopleWave8ValidationError(ValueError):
    """Raised when Wave 8 violates scope, evidence, or integrity rules."""


def _fail(message: str) -> None:
    raise Stage3DFillProgramPeopleWave8ValidationError(message)


def validate_preflight_state(current_head: str, status_short: str, expected_head: str) -> None:
    """Fail closed if the operator starts from a dirty or stale Wave 7 baseline."""
    if status_short.strip():
        _fail("Wave 8 preflight requires a clean worktree")
    if current_head.strip() != expected_head.strip():
        _fail("Wave 8 preflight HEAD does not match the expected Wave 7 baseline")
...
def _read(path: Path) -> dict[str, Any]:
    try:
        ...
        _fail(f"Cannot read Wave 8 input {path}: {error}")
...
def _verify_pins(
    ...
    expected = {
        ...
        "wave8_source_manifest_input": Path(source_manifest_path).resolve(),
        "wave8_cache_manifest_input": Path(cache_manifest_path).resolve(),
        "wave8_observations_input": Path(observations_path).resolve(),
        "wave8_exclusions_input": Path(exclusions_path).resolve(),
    }
    for pin_id, path in expected.items():
        if pin_id not in pins or Path(pins[pin_id]["resolved_path"]).resolve() != path:
            _fail(f"Wave 8 argument does not match immutable pin {pin_id}")
    preflight = document.get("preflight")
    if not isinstance(preflight, dict):
        _fail("Wave 8 immutable manifest requires a recorded preflight")
    ...
    if (
        preflight.get("expected_wave7_head") != EXPECTED_WAVE7_HEAD
        ...
        _fail("Wave 8 preflight record is incomplete or the Stage 3A stash was touched")
    descriptors = document.get("prior_wave_inputs")
    expected_waves = ["wave1", "wave2", "wave3", "wave4", "wave5", "wave6", "wave7"]
    if not isinstance(descriptors, list) or [row.get("wave_id") for row in descriptors] != expected_waves:
        _fail("Wave 8 requires ordered immutable Wave 1 through Wave 7 inputs")
    for row in descriptors:
        if row.get("program_people_pin_id") not in pins or row.get("summary_pin_id") not in pins:
            _fail("Wave 8 prior-wave descriptor has an unresolved immutable pin")
        if row.get("collection_field") != "slots":
            _fail("Wave 8 prior-wave scope must be derived from slot artifacts")
    if document.get("prior_cumulative_pin_id") not in pins:
        _fail("Wave 8 prior cumulative program-person pin is unresolved")
...
def _candidate_scope(candidate_path: Path) -> dict[str, dict[str, Any]]:
    ...
    if (
        ...
        _fail("Wave 8 requires the immutable 62-school Candidate v2 scope")
    return candidates
...
def _prior_wave_state(
    ...
    for descriptor in pin_document["prior_wave_inputs"]:
        ...
        if len(rows) != expected_lengths[wave_id]:
            _fail(f"Wave 8 pinned {wave_id} slot count is invalid")
        if any(row.get("candidate_id") not in candidates for row in rows):
            _fail(f"Wave 8 pinned {wave_id} contains a school outside Candidate v2")
        if wave_id in {"wave4", "wave5", "wave6", "wave7"}:
            if len(base_slots) != TOTAL_PROGRAM_SLOTS:
                _fail("Wave 8 must load the complete Waves 1-3 inventory before overlays")
            for row in rows:
                ...
                if (
                    ...
                    _fail(f"Wave 8 {wave_id} overlay must target a prior pending slot")
                base_slots[slot_id] = row
        else:
            for row in rows:
                slot_id = row.get("slot_id")
                if not slot_id or slot_id in base_slots:
                    _fail("Wave 8 Waves 1-3 must provide unique base slots")
                base_slots[slot_id] = row
    ...
    if len(all_slots) != TOTAL_PROGRAM_SLOTS:
        _fail("Wave 8 prior waves must provide exactly 310 unique program slots")
    if statuses != Counter(
        {
        ...
        _fail("Wave 8 baseline must be 100 identified and 210 pending")
...
def _program_inventory(
    ...
    for candidate_id, candidate in sorted(candidates.items()):
        ...
        if len(programs) != 5:
            _fail(f"Wave 8 requires five immutable demo programs for {candidate_id}")
        for index, program in enumerate(programs, 1):
            if (
                ...
                _fail("Wave 8 demo-program provenance is incomplete")
    ...
    if len(inventory) != TOTAL_PROGRAM_SLOTS:
        _fail("Wave 8 immutable demo-program inventory must contain 310 slots")
    return inventory
...
def _select_slots(
    ...
    for prior in prior_slots:
        ...
        if not slot:
            _fail("Wave 8 pending slot does not resolve to the demo-program inventory")
        slot["priority_tier"] = _priority_tier(
            slot["program_name"], slot["normalized_program_name"]
        )
        pending.append(slot)
    if len(pending) < ATTEMPTED_SLOTS:
        _fail("Wave 8 lacks 100 remaining program slots")
    ...
    for tier in (1, 2, 3):
        ...
        if len(selected) == ATTEMPTED_SLOTS:
            break
    if len(selected) != ATTEMPTED_SLOTS:
        _fail("Wave 8 must attempt exactly 100 high-yield pending slots")
    return selected
...
def _source_cache(
    ...
    if source_doc.get("record_type") != "stage3d_fill_program_people_wave8_source_manifest_input":
        _fail("Wave 8 source manifest has an invalid record type")
    if (
        cache_doc.get("record_type")
        != "stage3d_fill_program_people_wave8_cache_manifest_input"
        or cache_doc.get("cache_is_gitignored") is not True
    ):
        _fail("Wave 8 cache manifest must declare a gitignored reviewed cache")
    ...
    for original in source_doc.get("sources", []):
        ...
        if (
            ...
            _fail("Wave 8 source IDs must be unique and resolve to selected schools")
        _reject_detail_ranking_fields(row, "wave8_source")
        if (
            row.get("source_type") != "official_institutional"
            or not row.get("source_url")
            or not row.get("publisher")
        ):
            _fail("Wave 8 positive sources must be complete official institutional sources")
        ...
        if parsed_url.scheme != "https" or not any(
            ...
            _fail("Wave 8 source host must match the candidate's official institution")
        ...
        if (
            ...
            _fail("Wave 8 sources require short verified direct quotes")
...
def _apply_observations(
    ...
    if (
        ...
        != "stage3d_fill_program_people_wave8_program_people_observations"
    ):
        _fail("Wave 8 observations have an invalid record type")
    selected_by_id = {row["slot_id"]: row for row in selected}
    observations: dict[str, dict[str, Any]] = {}
    for original in document.get("observations", []):
        row = deepcopy(original)
        _reject_detail_ranking_fields(row, "wave8_observation")
        slot_id = row.get("slot_id")
        ...
        if (
            ...
            _fail("Wave 8 observation must resolve exactly once to a selected slot")
        status = row.get("slot_status")
        if status not in SLOT_STATUSES:
            _fail("Wave 8 slot status is invalid")
        if status == "no_qualifying_person_found":
            ...
            if not row.get("reviewed_scope") or not reviewed_source_ids:
                _fail("Wave 8 no_qualifying_person_found requires reviewed scope and sources")
            if any(
                ...
                _fail(
                    "Wave 8 no_qualifying_person_found requires cache-verified "
                    "reviewed sources for the same school"
                )
        elif status == "identified_person":
            source_ids = row.get("source_ids")
            if not isinstance(source_ids, list) or not source_ids:
                _fail("Wave 8 identified person requires source-backed dual evidence")
            if any(
                source_id not in sources
                or sources[source_id].get("candidate_id") != row["candidate_id"]
                for source_id in source_ids
            ):
                _fail("Wave 8 identified source does not resolve to the same school")
            ...
            for source_id, quote, role in (
                ...
                if (
                    ...
                    _fail(f"Wave 8 identified {role} quote is absent from reviewed cache")
            if row.get("relationship_type") not in ALLOWED_RELATIONSHIPS:
                _fail("Wave 8 identified person uses a forbidden attendance relationship")
            if row.get("match_type") not in ALLOWED_MATCH_TYPES:
                _fail("Wave 8 identified person uses an invalid program match")
            if row.get("program_match_basis") not in ALLOWED_MATCH_BASES:
                _fail(
                    "Wave 8 program match must be source-stated, never inferred from "
            ...
            if (
                ...
                _fail("Wave 8 identified person requires a non-empty resolved name")
            if not row.get("match_notes") or len(str(row["match_notes"])) > MAX_NOTES_LENGTH:
                _fail("Wave 8 program match requires concise notes")
            identity = {
                **row,
                "person_identity_disambiguator_source_id": source_ids[0],
            }
            if row.get("person_id") != _expected_person_id(identity):
                _fail("Wave 8 person ID must include name, candidate, and source context")
...
def _deduplicate(
    ...
    for row in occurrences:
        ...
        if not all(key):
            _fail("Wave 8 cumulative program person lacks a dedup key")
    ...
    for key, rows in sorted(grouped.items()):
        ...
        if len(names) != 1:
            _fail("Wave 8 cumulative duplicate key maps to different people")
    ...
    if duplicates:
        _fail("Wave 8 introduces a duplicate candidate/person key")
    keys = [(row["candidate_id"], row["canonical_person_id"]) for row in merged]
    if len(keys) != len(set(keys)):
        _fail("Wave 8 cumulative output retains duplicate person keys")
...
def _validate_prior_cumulative_people(
    ...
    if (
        ...
        _fail(
            "Wave 8 pinned cumulative people must match the 100 identified "
            "Waves 1-7 slot records exactly"
        )


def build_stage3d_fill_program_people_wave8(
    *,
    candidate_path: Path,
    programs_path: Path,
    ...
    """Build the independent Wave 8 overlay from pinned Waves 1-7."""
    ...
    if exclusions_doc.get("record_type") != "stage3d_fill_program_people_wave8_exclusions_input":
        _fail("Wave 8 exclusions input has an invalid record type")
    exclusions = deepcopy(exclusions_doc.get("exclusions", []))
    selected_by_id = {row["slot_id"]: row for row in selected}
    exclusion_keys: set[tuple[str, str, str]] = set()
    for row in exclusions:
        _reject_detail_ranking_fields(row, "wave8_exclusion")
        if row.get("exclusion_reason") not in ALLOWED_EXCLUSIONS:
            _fail("Wave 8 exclusion reason is invalid")
        required = (
            "slot_id",
            "candidate_id",
            "person_name",
        ...
        if any(not row.get(field) for field in required):
            _fail("Wave 8 exclusion is missing required provenance fields")
        ...
        if (
            ...
            _fail("Wave 8 exclusion must resolve to a selected same-school reviewed source")
        ...
        if (
            ...
            _fail("Wave 8 exclusion evidence is absent from the reviewed cache")
        key = (row["candidate_id"], row["person_name"], row["exclusion_reason"])
        if key in exclusion_keys:
            _fail("Wave 8 exclusion records must be unique")
    ...
    if len(cumulative_by_slot) != TOTAL_PROGRAM_SLOTS:
        _fail("Wave 8 cumulative dashboard must account for all 310 slots")
    if (
        len(cumulative_people) != cumulative_statuses["identified_person"]
        or raw_occurrences
        != PRIOR_IDENTIFIED_SLOTS + wave_statuses["identified_person"]
    ):
        _fail("Wave 8 cumulative people and slot-derived identified counts diverge")
    prior_counts = {
    ...
    if prior_counts != {
        ...
        _fail("Wave 8 prior summaries do not match the 100-person baseline")
    source_output = [
        {
            **sources[source_id],
    ...
    summary = _flags(
        "stage3d_fill_program_people_wave8_summary",
        ...
        wave8_identified_count=wave_statuses["identified_person"],
        newly_identified_person_count=wave_statuses["identified_person"],
        wave8_source_review_not_completed_count=wave_statuses[
            "source_review_not_completed"
        ],
        attempted_source_review_not_completed_count=wave_statuses[
            "source_review_not_completed"
        ],
        wave8_no_qualifying_person_found_count=wave_statuses[
        ...
        coverage_delta_from_wave7=wave_statuses["identified_person"],
        ...
        not_final_reason=(
            "Wave 8 expands high-yield reviewed program-person coverage but "
    ...
    return {
        "stage3d-fill-program-people-wave8-plan.json": _flags(
            "stage3d_fill_program_people_wave8_plan",
            remaining_slots_before_wave8=PRIOR_PENDING_SLOTS,
            tier1_remaining_slots_before_wave8=sum(
                1
                for row in prior_slots
            ...
            derivation=(
                "Waves 1-7 cumulative source_review_not_completed slots joined to "
        ...
        "stage3d-fill-program-people-wave8-program-people.json": _flags(
            "stage3d_fill_program_people_wave8_program_people",
            slots=slots,
        ),
        "stage3d-fill-program-people-wave8-exclusions.json": _flags(
            "stage3d_fill_program_people_wave8_exclusions",
            exclusions=exclusions,
        ),
        "stage3d-fill-program-people-wave8-source-manifest.json": _flags(
            "stage3d_fill_program_people_wave8_source_manifest",
            sources=source_output,
        ),
        "stage3d-fill-program-people-wave8-cache-manifest.json": _flags(
            "stage3d_fill_program_people_wave8_cache_manifest",
            cache_is_gitignored=True,
            entries=cache_output,
        ),
        "stage3d-fill-program-people-wave8-gap-disclosure.json": _flags(
            "stage3d_fill_program_people_wave8_gap_disclosure",
            gaps=gaps,
            attempted_source_review_not_completed_count=wave_statuses[
                "source_review_not_completed"
            ],
        ...
        "stage3d-fill-program-people-wave8-cumulative-dedup.json": _flags(
            "stage3d_fill_program_people_wave8_cumulative_dedup",
        ...
        "stage3d-fill-program-people-wave8-dedup-report.json": _flags(
            "stage3d_fill_program_people_wave8_dedup_report",
            dedup_key_fields=["candidate_id", "canonical_person_id"],
            raw_person_occurrence_count=raw_occurrences,
            unique_person_count=len(cumulative_people),
            duplicate_person_count=len(duplicate_records),
        ...
        "stage3d-fill-program-people-wave8-summary.json": summary,
    }


def validate_stage3d_fill_program_people_wave8(
    artifacts: dict[str, dict[str, Any]],
    **inputs: Any,
) -> dict[str, Any]:
    """Fail closed when Wave 8 diverges from reviewed deterministic inputs."""
    if set(artifacts) != set(OUTPUT_FILES):
        _fail("Wave 8 artifact set is incomplete")
    expected = build_stage3d_fill_program_people_wave8(**inputs)
    if artifacts != expected:
        _fail("Wave 8 artifacts do not match deterministic regeneration")
    plan = artifacts["stage3d-fill-program-people-wave8-plan.json"]
    slots = artifacts["stage3d-fill-program-people-wave8-program-people.json"][
        "slots"
    ]
    summary = artifacts["stage3d-fill-program-people-wave8-summary.json"]
    dedup = artifacts["stage3d-fill-program-people-wave8-dedup-report.json"]
    if len(plan["selected_slots"]) != ATTEMPTED_SLOTS or len(slots) != ATTEMPTED_SLOTS:
        _fail("Wave 8 must deterministically attempt 100 remaining slots")
    if any(row["priority_tier"] not in {1, 2, 3} for row in slots):
        _fail("Wave 8 selection contains an invalid program-priority tier")
    ...
    if plan.get("priority_tier_counts") != dict(sorted(expected_tiers.items())):
        _fail("Wave 8 plan priority counts do not match selected slots")
    if any(row["slot_status"] not in SLOT_STATUSES for row in slots):
        _fail("Wave 8 contains an invalid slot status")
    if summary["cumulative_program_slots_processed"] != TOTAL_PROGRAM_SLOTS:
        _fail("Wave 8 cumulative dashboard must retain 310 processed slots")
    if summary["cumulative_total_program_slots"] != TOTAL_PROGRAM_SLOTS:
        _fail("Wave 8 cumulative total-program-slot count must remain 310")
    if sum(
        summary[key]
        for key in (
        ...
        _fail("Wave 8 cumulative dashboard does not sum to 310 slots")
    keys = [
    ...
    if (
        ...
        _fail("Wave 8 cumulative output retains duplicate keys")
    if (
        summary["manual_verbatim_check_count"] != 0
        or summary["cache_missing_count"] != 0
    ):
        _fail("Wave 8 requires complete local-cache verification")
    if (
        summary["source_policy_violations"] != 0
        or summary["ranking_field_contamination"] != 0
    ):
        _fail("Wave 8 source policy or ranking isolation failed")
    if summary["readiness_status"] != "source_limited / incomplete / not_final":
        _fail("Wave 8 readiness must remain source-limited, incomplete, and not-final")
    if any(
        summary[field]
        for field in (
        ...
        _fail("Wave 8 must not generate final, membership, or frontend exports")
    return {
        "record_type": "stage3d_fill_program_people_wave8_validation_result",
        "status": "passed",
        **FLAGS,
        "checks_passed": 25,
        "slots_attempted": len(slots),
        "identified_people": summary["wave8_identified_count"],
        "source_review_not_completed": summary[
            "wave8_source_review_not_completed_count"
        ],
        "no_qualifying_person_found": summary[
            "wave8_no_qualifying_person_found_count"
        ],
...
def validate_stage3d_fill_program_people_wave8_output_path(
    artifact_dir: Path,
    ...
    if Path(auxiliary_output).resolve() in forbidden:
        _fail("Wave 8 auxiliary output must not overwrite an artifact")


def validate_stage3d_fill_program_people_wave8_committed_result(
    artifact_dir: Path,
    ...
    if not path.is_file() or _read(path) != expected:
        _fail("Wave 8 committed validation result does not match validator rerun")


def validate_stage3d_fill_program_people_wave8_artifact_directory(
    artifact_dir: Path,
    *,
    ...
    if not present.issubset(expected):
        _fail("Wave 8 artifact directory contains unexpected files")
    if require_complete and present != expected:
        _fail("Wave 8 artifact directory is incomplete")


def write_stage3d_fill_program_people_wave8(
    artifacts: dict[str, dict[str, Any]],
    output_dir: Path,
...
def render_stage3d_fill_program_people_wave8_report(
    artifacts: dict[str, dict[str, Any]],
) -> str:
    summary = artifacts["stage3d-fill-program-people-wave8-summary.json"]
    return f"""# Stage 3D-Fill Program People Coverage Expansion Wave 8 Report
...
- newly identified people: **{summary['wave8_identified_count']}**
- attempted slots still source review not completed: **{summary['wave8_source_review_not_completed_count']}**
- no qualifying person found after scoped review: **{summary['wave8_no_qualifying_person_found_count']}**
- exclusions: **{summary['exclusions_count']}**

Wave 8 derives its scope from immutable Waves 1-7 and prioritizes remaining Tier 1 and Tier 2 program families with school-spread round-robin selection. Processed slots are not presented as identified people.
...
- coverage delta from Wave 7: **+{summary['coverage_delta_from_wave7']}**
...
- upstream Waves 1-7 artifacts modified: **false**
