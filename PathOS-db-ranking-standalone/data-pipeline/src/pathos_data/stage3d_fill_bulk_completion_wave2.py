"""Reviewed top-five program-person intake for Bulk Completion Wave 2."""
...
PIN_RECORD_TYPE = "stage3d_fill_bulk_completion_wave2_immutable_input_pin_manifest"
OUTPUT_FILES = (
    "stage3d-fill-bulk-completion-wave2-plan.json",
    "stage3d-fill-bulk-completion-wave2-slot-inventory.json",
    "stage3d-fill-bulk-completion-wave2-program-people.json",
    "stage3d-fill-bulk-completion-wave2-cumulative-program-people.json",
    "stage3d-fill-bulk-completion-wave2-duplicate-records.json",
    "stage3d-fill-bulk-completion-wave2-exclusions.json",
    "stage3d-fill-bulk-completion-wave2-source-manifest.json",
    "stage3d-fill-bulk-completion-wave2-cache-manifest.json",
    "stage3d-fill-bulk-completion-wave2-gap-disclosure.json",
    "stage3d-fill-bulk-completion-wave2-summary.json",
)
VALIDATION_FILE = "stage3d-fill-bulk-completion-wave2-validation-result.json"
...
class Stage3DFillBulkCompletionWave2ValidationError(ValueError):
    """Raised when Wave 2 violates scope, evidence, or integrity rules."""


def _fail(message: str) -> None:
    raise Stage3DFillBulkCompletionWave2ValidationError(message)
...
def _read(path: Path) -> dict[str, Any]:
    try:
        ...
        _fail(f"Cannot read Wave 2 input {path}: {error}")
...
def _candidate_scope(candidate_path: Path) -> dict[str, dict[str, Any]]:
    ...
    if len(rows) != 62 or len(candidates) != 62 or None in candidates:
        _fail("Wave 2 requires the immutable 62-school Candidate v2 scope")
    return candidates
...
def _verify_pins(
    ...
    for pin_id, path in expected.items():
        if pin_id not in pins or Path(pins[pin_id]["resolved_path"]).resolve() != path:
            _fail(f"Wave 2 argument does not match immutable pin {pin_id}")
    batches = document.get("program_person_batches")
    if not isinstance(batches, list) or {row.get("batch_id") for row in batches} != {
        "stage3d-fill-bulk-completion-wave1-cumulative",
    }:
        _fail("Wave 2 prior program-person batch manifest is incomplete")
    for row in batches:
        if row.get("artifact_pin_id") not in pins or row.get("collection_field") not in {"records", "slots"}:
            _fail("Wave 2 program-person batch pin is unresolved")
...
def _school_scope(
    ...
    if document.get("record_type") != "stage3d_fill_bulk_completion_wave2_school_manifest":
        _fail("Wave 2 school manifest has an invalid record type")
    ...
    if len(ids) != 20 or len(set(ids)) != 20 or None in ids or not set(ids) <= set(candidates):
        _fail("Wave 2 school manifest must contain 20 distinct Candidate v2 schools")
...
def _program_inventory(
    ...
    for candidate in schools:
        ...
        if len(program_rows) != 5:
            _fail(f"Wave 2 requires five immutable demo programs for {candidate_id}")
        for index, program in enumerate(program_rows, 1):
            if not program.get("program_name") or not program.get("normalized_program_name") or not program.get("source_id"):
                _fail("Wave 2 demo-program provenance is incomplete")
            inventory.append({
...
def _source_cache(
    ...
    if source_doc.get("record_type") != "stage3d_fill_bulk_completion_wave2_source_manifest_input":
        _fail("Wave 2 source manifest has an invalid record type")
    if (
        cache_doc.get("record_type") != "stage3d_fill_bulk_completion_wave2_cache_manifest_input"
        or cache_doc.get("cache_is_gitignored") is not True
    ):
        _fail("Wave 2 cache manifest must declare a gitignored reviewed cache")
    ...
    for original in source_doc.get("sources", []):
        ...
        if not source_id or source_id in sources or row.get("candidate_id") not in candidate_ids:
            _fail("Wave 2 source IDs must be unique and in scope")
        _reject_detail_ranking_fields(row, "wave2_source")
        if row.get("source_type") != "official_institutional" or not row.get("source_url") or not row.get("publisher"):
            _fail("Wave 2 positive sources must be complete official institutional sources")
        ...
        if not isinstance(quotes, list) or not quotes or any(
            ...
            _fail("Wave 2 sources require short verified direct quotes")
...
def _apply_observations(
    ...
    if document.get("record_type") != "stage3d_fill_bulk_completion_wave2_program_people_observations":
        _fail("Wave 2 observations have an invalid record type")
    inventory_by_id = {row["slot_id"]: row for row in inventory}
    observations = {}
    for original in document.get("observations", []):
        row = deepcopy(original)
        _reject_detail_ranking_fields(row, "wave2_observation")
        slot_id = row.get("slot_id")
        if slot_id not in inventory_by_id or slot_id in observations:
            _fail("Wave 2 observation must resolve exactly once to an approved slot")
        ...
        if status == "no_qualifying_person_found":
            ...
            if not source or not cache or source.get("candidate_id") != row.get("candidate_id"):
                _fail("Wave 2 identified source does not resolve to the same school")
            if any(
                ...
                _fail("Wave 2 identified attendance or program quote is absent from the reviewed source cache")
            if row.get("relationship_type") not in ALLOWED_RELATIONSHIPS:
                _fail("Wave 2 identified person uses a forbidden attendance relationship")
            if row.get("match_type") not in ALLOWED_MATCH_TYPES:
                _fail("Wave 2 identified person uses an invalid program match")
            if row.get("program_match_basis") not in ALLOWED_MATCH_BASES:
                _fail("Wave 2 program match must be source-stated, never inferred from career or fame")
            if not row.get("match_notes") or len(str(row["match_notes"])) > MAX_NOTES_LENGTH:
                _fail("Wave 2 program match requires concise notes")
            identity = {
                **row,
                "person_identity_disambiguator_source_id": source_id,
            }
            if row.get("person_id") != _expected_person_id(identity):
                _fail("Wave 2 person ID must include name, candidate, and source context")
...
def _deduplicate_program_people(
    ...
    for row in wave_rows:
        if row["slot_status"] == "identified_person":
            clone = deepcopy(row)
            clone["origin_batch"] = "stage3d-fill-bulk-completion-wave2"
            all_rows.append(clone)
    grouped: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for row in all_rows:
        key = (row.get("candidate_id"), row.get("person_id"))
        if not all(key):
            _fail("Wave 2 cumulative program person lacks a dedup key")
    ...
    for key, rows in sorted(grouped.items()):
        ...
        if len(names) != 1:
            _fail("Wave 2 cumulative duplicate key maps to different person names")
    ...
    if len(keys) != len(set(keys)):
        _fail("Wave 2 cumulative program-person output retains duplicate keys")
    return merged, duplicates


def build_stage3d_fill_bulk_completion_wave2(
    *,
    candidate_path: Path,
    programs_path: Path,
    ...
    """Build the independent Wave 2 top-five program-person overlay."""
    ...
    if exclusions_doc.get("record_type") != "stage3d_fill_bulk_completion_wave2_exclusions_input":
        _fail("Wave 2 exclusions input has an invalid record type")
    exclusions = deepcopy(exclusions_doc.get("exclusions", []))
    for row in exclusions:
        _reject_detail_ranking_fields(row, "wave2_exclusion")
        if row.get("exclusion_reason") not in ALLOWED_EXCLUSIONS:
            _fail("Wave 2 exclusion reason is invalid")
    ...
    summary = _flags(
        "stage3d_fill_bulk_completion_wave2_summary",
        total_wave2_universities=20,
        schools_processed_count=20,
        attendance_records_added_count=0,
        attendance_evidence_reused_count=statuses["identified_person"],
        ...
        wave2_duplicate_person_count=len(duplicate_records),
        ...
        not_final_reason="Wave 2 covers 20 Candidate v2 schools and five existing demo-program slots per school; it remains source-limited and incomplete.",
    )
    source_output = []
    cache_output = []
    ...
    return {
        "stage3d-fill-bulk-completion-wave2-plan.json": _flags(
            "stage3d_fill_bulk_completion_wave2_plan",
            university_count=20,
        ...
        "stage3d-fill-bulk-completion-wave2-slot-inventory.json": _flags(
            "stage3d_fill_bulk_completion_wave2_slot_inventory", slots=inventory,
        ),
        "stage3d-fill-bulk-completion-wave2-program-people.json": _flags(
            "stage3d_fill_bulk_completion_wave2_program_people", slots=slots,
        ),
        "stage3d-fill-bulk-completion-wave2-cumulative-program-people.json": _flags(
            "stage3d_fill_bulk_completion_wave2_cumulative_program_people",
            dedup_key_fields=["candidate_id", "canonical_person_id"], records=cumulative,
        ),
        "stage3d-fill-bulk-completion-wave2-duplicate-records.json": _flags(
            "stage3d_fill_bulk_completion_wave2_duplicate_records", duplicate_records=duplicate_records,
        ),
        "stage3d-fill-bulk-completion-wave2-exclusions.json": _flags(
            "stage3d_fill_bulk_completion_wave2_exclusions", exclusions=exclusions,
        ),
        "stage3d-fill-bulk-completion-wave2-source-manifest.json": _flags(
            "stage3d_fill_bulk_completion_wave2_source_manifest", sources=source_output,
        ),
        "stage3d-fill-bulk-completion-wave2-cache-manifest.json": _flags(
            "stage3d_fill_bulk_completion_wave2_cache_manifest", cache_is_gitignored=True, entries=cache_output,
        ),
        "stage3d-fill-bulk-completion-wave2-gap-disclosure.json": _flags(
            "stage3d_fill_bulk_completion_wave2_gap_disclosure",
            gaps=gap_rows,
            source_review_not_completed_is_not_none=True,
        ),
        "stage3d-fill-bulk-completion-wave2-summary.json": summary,
    }


def validate_stage3d_fill_bulk_completion_wave2(
    artifacts: dict[str, dict[str, Any]], **inputs: Any,
) -> dict[str, Any]:
    """Fail closed when Wave 2 diverges from deterministic source inputs."""
    if set(artifacts) != set(OUTPUT_FILES):
        _fail("Wave 2 artifact set is incomplete")
    expected = build_stage3d_fill_bulk_completion_wave2(**inputs)
    if artifacts != expected:
        _fail("Wave 2 artifacts do not match deterministic regeneration")
    ...
    if len(slots) != 100 or summary["program_slots_processed_count"] != 100:
        _fail("Wave 2 must process exactly 100 slots")
    if len(keys) != len(set(keys)) or summary["cumulative_post_merge_duplicate_count"] != 0:
        _fail("Wave 2 cumulative program-person output contains duplicate keys")
    if summary["source_policy_violations"] != 0 or summary["ranking_field_contamination"] != 0:
        _fail("Wave 2 source policy or ranking isolation failed")
    return {
        "record_type": "stage3d_fill_bulk_completion_wave2_validation_result",
        "status": "passed",
        **FLAGS,
        "checks_passed": 24,
...
def write_stage3d_fill_bulk_completion_wave2(
...
def render_stage3d_fill_bulk_completion_wave2_report(
    artifacts: dict[str, dict[str, Any]],
) -> str:
    summary = artifacts["stage3d-fill-bulk-completion-wave2-summary.json"]
    return f"""# Stage 3D-Fill Bulk Completion Wave 2 Report

## Reviewed program-person intake

- Candidate v2 schools processed: **{summary['schools_processed_count']}**
- new notable-attendance records added: **{summary['attendance_records_added_count']}**
- immutable attendance evidence reused: **{summary['attendance_evidence_reused_count']}**
...
- identified input occurrences across the pinned Wave 1 cumulative layer and Wave 2: **{summary['cumulative_input_identified_occurrence_count']}**
...
Immutable upstream inputs are protected by a versioned input pin manifest rather than hashes embedded in Python source. Wave 1 artifacts remain unchanged.
